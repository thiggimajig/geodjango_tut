...
def censusmap(request):
    ...
    return render(request,'maps/census_map.html',{'map':map})


def policyexplorer(request):
    # maps = call_all_pf_functions()
    # maps = config.call_all_pf_functions()
    map = config.map_orig
    map = map._repr_html_()
...
#     host_count_unique = len(host_count)
    ...
    return render(request, "maps/policy2.html",{'map':map, 'stats': stats})
...
#     host_count_unique = len(host_count)
    ...
    return render(request, "maps/policy3.html",{'map':map, 'stats': stats})
